...
- (void)performRequest:(id<VDFRequest>)request {
    @synchronized(self.pendingRequests) {
        ...
        if([[VDFSettings sharedCacheManager] isResponseCachedForRequest:request]) {
            ...
            NSData *response = [[VDFSettings sharedCacheManager] responseForRequest:request];
            [request onDataResponse:response];
        }
        else {
            ...
            if(!requestIsPending) {
                ...
                if([[request httpMethod] isEqualToString:@"POST"]) {
                    [httpRequest post:requestUrl withBody:[request postBody]];
                }
                else {
...
- (void)retrayRequest:(id<VDFRequest>)request {
    // TODO
    // make some http pooling or retray in some seconds
}
...
- (void)httpRequest:(VDFHttpRequest*)request onResponse:(NSData*)data {
    @synchronized(self.pendingRequests) {
        ...
        if(correspondingRequest != nil) {
            [[VDFSettings sharedCacheManager] cacheResponseData:data forRequest:correspondingRequest];
        }
